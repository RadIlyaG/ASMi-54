...
proc GetPageFile {barcode {trac {}}} {
  ...
  puts "Get Pages 0-3 for $barcode $trac"
  Status "Wait for Pages 0-3 retrieval ..."
  #------------
  # demo:  
  ...
  } elseif {$gaSet(pageType) == "Local"} {
    set fileName $gaSet(localPageFile)
  }
 ...
 if {[string match *error* [string tolower $res]]} {
   set gaSet(fail) "Page file error"
   puts $res ; update
   return -1
 }
...
proc WritePages {uut} {
  ...
  set com $gaSet(com$uut)
	...
	if {[Send $com "\r" "\[boot" 1] != 0} {
	  set gaSet(fail) "$uut - Failed to get Boot Menu" ; update
    return -1
	}
  ...
  if {$ret!=1} {
    set gaSet(fail) "$uut - Failed to get Device IP" ; update
    return -1	  
  }
  ...
  for {set page 0} {$page <=3} {incr page 1} {
		...
		set offSet 00   
    Status "$uut - Writing page $page"   	
		
		#Write:
    if {[Send $com "c2 $device,0$page,$offSet,$gaGet(page$page),$crc\r" "data ?" 3] != 0} {
		  set gaSet(fail) "$uut - Writing Error - page $page"
      return -1
    }			      
    ...
    if {$ret!=1} {
      set gaSet(fail) "$uut - Page$page check fail." ; update
      return -1	  
    }
    ...
    if {[string match *$gaGet(page$page)* $res]==0} {
      set gaSet(fail) "$uut - Page$page result fail." ; update
      puts "$uut - res:$res"
      puts "$uut - pag:$gaGet(page$page)"
